...
public class BlockChain {
  ...
  public BlockChain(int initial) {
    Block firstBlock;
    firstBlock = new Block(0, initial, null);
  ...
  public Block mine(int amount) throws NoSuchAlgorithmException {
    Block newBlock = new Block(this.curNode.data.getNum() + 1, amount, this.curNode.data.getHash());
  ...
  public boolean isValidBlockChain() {
    ...
    for (this.curNode = this.front; this.curNode != null; this.curNode = this.curNode.next) {
      if (this.curNode == this.front) {
        if (this.curNode.data.amount < 0) {
          this.curNode = temp;
          return false;
        }
      }

      if (!this.curNode.data.curHash.isValid()) {
        this.curNode = temp;
        return false;
      }

      if (this.curNode != this.front && this.curNode.next != null) {
        if (this.curNode.data.curHash != this.curNode.next.data.getPrevHash()) {
          this.curNode = temp;
          return false;
        }
      }

      total = total + this.curNode.data.amount;
      if (total < 0) {
        this.curNode = temp;
        return false;
      }
  ...
  public void printBalances() {
    Node temp = this.curNode;
    this.curNode = this.front;
    int blake = this.curNode.data.getAmount();
    int alexis = 0;

    for (this.curNode = this.front; this.curNode != null; this.curNode = this.curNode.next) {
      if (this.curNode.data.getAmount() >= 0) {
        alexis += this.curNode.data.getAmount();
        blake -= this.curNode.data.getAmount();
    ...
    System.out.println("Alexis: " + alexis + ", Blake: " + blake);
    this.curNode = temp;
  } // printBalances()
